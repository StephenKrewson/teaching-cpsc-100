...
int main(int argc, char* argv[])
{
    ...
    // ensure port is non-negative short, path to server's root is specified
    if (port < 0 || 
        port > SHRT_MAX || 
        argv[optind] == NULL || 
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // Method SP absolute-path ["?" query] SP HTTP-version CRLF
            // a. Method MUST come first and can only have value "GET"
            char* val_path = strstr(line, "GET ");
            if (val_path != line)
            {
                error(405);
                continue;
            }
            // b. Grab the absolute path, which MUST follow a single space
            if ((val_path = strchr(line, ' ')) == NULL)
            {
                error(400);
                continue;
            }
            // c. Advance absolute path pointer past the space
            val_path += 1;

            // d. Absoute path must begin with '/' else "Not Implemented" error
            if (val_path[0] != '/')
            {
                error(501);
                continue;
            ...
            // f. Move on to the http validation; ensure some substring exists
            char* val_http = strchr(val_path, ' ');
            if (val_http == NULL)
            {
                error(505);
                continue;
            }
            // g. If HTTP exists, advance the pointer past the space
            val_http += 1;
                
            // h. It must be type 1.1 and not be followed by whitespace/junk
            if (strcmp(val_http, "HTTP/1.1\r\n") != 0)
            {
                error(505);
                continue;
            }

            // Assume len_path where start_query is NULL
            int len_path = val_http - val_path - 1;
            
            // Find the start of the query, if any
            char* start_query = strchr(val_path, '?');

            // Now make room for query substring on the stack
            char query[strlen(val_path) + 1];
            
            // Initialize it with NULLs and copy into it if a '?' was found
            memset(query, '\0', sizeof(query));
            if (start_query != NULL)
            {
                len_path = start_query - val_path;
                // Advance past '?' and copy into query
                start_query += 1;
                strcpy(query, start_query);
                // Truncate query string
                query[val_http - start_query - 1] = '\0';
            }

            // e. Make sure path is well-formed; concatenate to root
            char path[strlen(root) + len_path + 1];
            strcpy(path, root);
            strncat(path, val_path, len_path);

            // Make sure path and query don't contain quote marks
            if ((strchr(path, '"') != NULL) || (strchr(query, '"') != NULL))
            {
                error(400);
                continue;
            }
            ...
            // Throw "not implemented" error if path lacks an extension
            char* start_ext = strrchr(path, '.');
            if (start_ext == NULL)
            {
                error(501);
                continue;
            ...
            start_ext += 1;
            char extension[strlen(start_ext) + 1];
            strcpy(extension, start_ext);
            ...
            if (strcasecmp("php", extension) == 0)
            {
                ...
                // subtract php-cgi's headers to get content length
                haystack = body;
...
/**
 * Accepts a connection from a client, blocking (i.e., waiting)
 * until one is heard. Upon success, returns true; upon failure,
 * returns false.
 */
bool connected(void)
{
...
const char* lookup(const char* extension)
{ 
    // Malloc a copy of the ext since we can't write to char* const
    char* copy = strdup(extension);
    ...
    // Handle the 7 valid options: css, html, gif, ico, jpg, js, png
