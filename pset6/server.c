...
char* indexes(const char* path)
{
    ...
    char* php_path = malloc(strlen(path) + strlen(php_ext) + 1);
    strcpy(php_path, path);
    strcat(php_path, php_ext);
    
    // First check for a PHP index (no need to create HTML string yet)
    if (access(php_path, F_OK) == 0)
        return php_path;
        
    // Then check for an HTML index
    char* html_path = malloc(strlen(path) + strlen(html_ext) + 1);
    strcpy(html_path, path);
    strcat(html_path, html_ext);
    
    if (access(html_path, F_OK) == 0)
        return html_path;

    // Avoid memory leak in default condition (no index found)
    free(php_path);
    free(html_path);
    return NULL;
}
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    // Initialize file info in case it's junk
    *content = NULL;
    *length = 0;

    // Use a standard buffer and a malloc'd storage chunk
    BYTE buffer[BYTES] = "";
    BYTE* storage = (BYTE*) malloc(BYTES);
    
    // Remember the return values from fread() calls
    size_t read_return = 0;
    
    // Take from the file stream until we can't
    while (!feof(file)) {
        
        // Read into buffer, update total size
        // Am assuming this doesn't fail weirdly
        // THIS is the idiomatic way to do it. Not getting one chunk of 512
        read_return = fread(buffer, sizeof(BYTE), BYTES, file);

        // Bad bug here not adding the "+1" cf. the code farther down
        // read_return is not # of bytes, but # of blocks
        storage = (BYTE*) realloc(storage, *length + read_return);
        
        // Now tack buffer contents onto end of storage
        // valgrind complains about storage being uninitialized...
        memcpy(storage + *length, buffer, read_return);
        
        // Don't forget to update *length
        // No need to null-terminate since storage size is exactly right
        *length += read_return;
    }
    
    // Now point content to our storage block, return false if it's empty
    *content = storage;
    if (*content == NULL)
        return false;
...
bool parse(const char* message, char* abs_path, char* query)
{
    ...
    // 1. Component extraction; overall format rules (error 400)
    // Get the request-line
    if ((target = strchr(message, ' ')) == NULL) {
        error(400);
        return false;
    }
    
    // Hop over the first SP, get the HTTP version
    target += 1;
    if ((version = strchr(target, ' ')) == NULL) {
        error(400);
        return false;
    }
    
    // Pause and make a copy of the request-target, for further parsing
    length_target = version - target;
    char request_target[length_target + 1];
    strncpy(request_target, target, length_target);
    request_target[length_target] = '\0';
    
    // Hop over second SP and figure out length of version
    // We only deal with version 1.1
    // so after the \r\n there's not necessarily a null byte?
    version += 1;
    char* allowed_version = "HTTP/1.1\r\n";
    if (strncmp(version, allowed_version, strlen(allowed_version)) != 0) {
        error(505);
        return false;
    }
    
    // 2. EXTRA checks on request-lines
    // Only valid type is "GET" followed by SP
    ...
    // Break out the "query"; make it the empty string if not exists
    char* request_query;
    if ((request_query = strchr(request_target, '?')) != NULL) {
        // Hope over '?'
        request_query += 1;
        int length_query = strlen(request_query);
        strncpy(query, request_query, length_query);
        query[length_query] = '\0';
        
        // check it doesn't have quote mark
        if (strchr(request_query, '"') != NULL) {
            error(400);
            return false;
        }
        
        // If all good, store the abs-path
        int length_target = request_query - request_target - 1; 
        strncpy(abs_path, request_target, length_target);
        abs_path[length_target] = '\0';
        
    } else {
        query[0] = '\0';
        strncpy(abs_path, request_target, strlen(request_target));
        abs_path[strlen(request_target)] = '\0';
    }

    // If all looks OK, return true
    return true;
}
